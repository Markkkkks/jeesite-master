...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public String getResult(String num1, String num2, char flag) throws Exception{
...
//		double n2 = Double.valueOf(num2);
		...
		if (n2.equals(BigDecimal.ZERO)&& flag == '/')
...
//			answer.setScale(2,BigDecimal.ROUND_HALF_UP);
			answer = n1.divide(n2,Math.max(Math.max(n1.scale(), n2.scale()), 2),BigDecimal.ROUND_HALF_UP);
			break;
		default :
			break;
		
		}
//		//结果格式化输出,整数正常输出，小数保留两位小数
...
//		}'
	...
	//返回计算器浏览记录列表，并加入到session
	public LinkedList<String> buildSession(String response, HttpServletRequest request) {
		// TODO Auto-generated method stub
		...
		if(session.getAttribute("list")==null)
			...
			list = (LinkedList<String>) session.getAttribute("list");
		if (list.size()>=3)
			list.removeLast();
		list.addFirst(response);
		session.setAttribute("list",list);
		return list;
	}

	//将计算记录插入到数据库中
	public void setCalData(User user, String response) {
		// TODO Auto-generated method stub
		Cal cal = new Cal();
		cal.setUserId(user.getId());
		cal.setData(response);
		cal.setCalculateDate(new Date());
		calDao.insertCal(cal);
	}
	public List<Cal> getCalHistory(User user){
		List<Cal> list =  calDao.findCalByUserId(user.getId());
	...
	//删除date
	public void deleteCalbyId(String Id) {
		// TODO Auto-generated method stub
		calDao.deleteCalbyId(Id);
	}
